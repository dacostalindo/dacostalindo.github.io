...
function begin_play(){
	...
	if (game_started()) {
		...
		if  (isEmpty(player1_name) || isEmpty(player2_name)) {
			    alert("Hey! Add the names of both players!");
		} else {
			started = true;
			var x = " (X)";
			var o = " (O)";
			var elem  = document.getElementById("player1_id");
			var old = elem.value;
			elem.value = old + x;

			var elem  = document.getElementById("player2_id");
			var old = elem.value;
			elem.value = old + o;
...
function reset_play(){
	...
	for (k=0; k<table_ids.length; k++) {
		document.getElementById(table_ids[k]).innerHTML = table_ids[k];
	}
	...
	if (game_started()){
		for (i=0; i<table_ids.length; i++) {
			if (move == table_ids[i]) {
				found = true;
				break
			} else if ( i == table_ids.length - 1) {
				alert("Please enter valid cell!");
			}
		}
	} else {
		...
		alert("Pick an empty cell!") ;
			...
			for (j =0; j<table_ids.length; j++) {
				...
				if (((j+1) % 3) ==1){
					...
					if (board_state[j] == 1){counter11 = counter11 + 1}
					if (board_state[j] == 0){counter10 = counter10 + 1}
				...
				if (((j+1) % 3) ==2){
					...
					if (board_state[j] == 1){counter21 = counter21 + 1}
					if (board_state[j] == 0){counter20 = counter20 + 1}
					...
					if (((j+1) % 3) ==0){
						...
						if (board_state[j] == 1){counter31 = counter31 + 1}
						if (board_state[j] == 0){counter30 = counter30 + 1}
